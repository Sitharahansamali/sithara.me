...
import { Compass, Rocket, Flag, Brain, Code, Cloud, CheckCircle2, Award } from 'lucide-react';
          ...
          <div className="space-y-12">
            {milestones.map((item, idx) => {
              ...
              return (
                ...
                >
                  {/* Timeline Dot Icon */}
                  <div className="absolute left-6 -translate-x-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full border-2 border-[#080b11] bg-[#0d1117] text-white shadow-[0_0_18px_rgba(88,166,255,0.5)] sm:left-1/2">
                    <Icon className="h-4 w-4" style={{ color: item.color }} />
                  </div>

                  {/* Card Content */}
                  <div className={`ml-14 w-full sm:ml-0 sm:w-1/2 ${isEven ? 'sm:pr-10 sm:text-right' : 'sm:pl-10 sm:ml-auto'}`}>
                    <div className="glass-panel group relative overflow-hidden rounded-3xl p-6 transition-all hover:border-[#58a6ff]/40 hover:-translate-y-1">
                      <div className="flex flex-wrap items-center gap-2 mb-2 sm:justify-end">
                        <span className="font-mono text-xs font-bold text-slate-400">MILESTONE {item.step}</span>
                        <span
                      ...
                      <h3 className="text-lg font-bold text-white group-hover:text-[#58a6ff] transition-colors" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                        {item.title}
                      </h3>
